...
```{r }
data <- xlsx::read.xlsx("/Users/eliane/Desktop/R Folder/coding_data.xlsx", sheetIndex = 1, endRow=52)

keys <- xlsx::read.xlsx("/Users/eliane/Desktop/R Folder/coding_data.xlsx", sheetIndex = 3, endRow=40)
...
## Step 6: end result is new_prac data frame 
...
new_prac <- first25 %>% 
  mutate(unite(across(everything(), ~ ifelse(. == "1", cur_column(), NA)),
               na.rm = T,
               col = "manual_heads")) %>%
...
```{r}
#interesting problem - why isn't nixon included?
# why is trump pays the schoo - headline 16
```
