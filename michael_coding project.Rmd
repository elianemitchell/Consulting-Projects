...
```{r }
data <- xlsx::read.xlsx("/Users/eliane/Desktop/R Folder/coding_data_three.xlsx", sheetIndex = 1, endRow=240)
...
prep_section <- prep_section[c(1:200), ]
...
prep_section$NLP_heads <- sapply(prep_section$response_lower, FUN = function(x){ 
  ...
  for (row in 1:nrow(keys_data)) { 
    ...
    if(row == 24) { 
      if(isTRUE(grepl(string_head, x)) & isTRUE(grepl("tax.*cut | tax cuts? | cutting tax | tax breaks? | tax refunds? | tas plan", response))) {
        matches <- paste(matches, row)
        next
      } else {
        matches <- paste(matches, "")
...
for (i in 1:nrow(section)) {
  ...
  for (row in 1:nrow(keys_data)) {
    ...
    if(row == 24) { 
      if(isTRUE(grepl(headline, response)) & isTRUE(grepl("tax.*cut | tax cuts? | cutting tax | tax breaks? | tax refunds? | tas plan", response))) {
        section[i, col] = 1
        next
      } else {
        section[i, col] = 0
...
```{r}
# To show the most common missing or added headlines
...
filtered <- filter(section3, grepl("26", missing))
