...
prep_section <- prep_section[c(101:150), ]
...
prep_section$NLP_heads <- sapply(prep_section$response_lower, FUN = function(x){ 
  ...
  for (row in 1:nrow(keys_data)) { 
    ...
    if(row == 24) { 
      if(isTRUE(grepl(string_head, x)) & isTRUE(grepl("tax.*cut | tax cuts? | cutting tax", x))) {
        matches <- paste(matches, row)
        next
      } else {
        matches <- paste(matches, "")
    ...
    if(row == 26) {
      if(isTRUE(grepl("hurt.*jobs | layoffs | less jobs | hurt employment | hurting employment | lower employment | jobs lost | less.*jobs | fewer.*jobs ", x)) & isTRUE(grepl("tarrifs? | tarriffs? | tariffs? | tarifs? | trade ", x))) {
        matches <- paste(matches, row)
        next
      } else {
        matches <- paste(matches, "")
...
for (i in 1:nrow(section)) {
  ...
  for (row in 1:nrow(keys_data)) {
    ...
    if(row == 24) { 
      if(isTRUE(grepl(headline, response)) & isTRUE(grepl("tax.*cut | tax cuts? | cutting tax", response))) {
        section[i, col] = 1
        next
      } else {
        section[i, col] = 0
    ...
    if(row == 26) {
      if(isTRUE(grepl("hurt.*jobs | layoffs | less jobs | hurt employment | hurting employment | lower employment | jobs lost | less.*jobs | fewer.*jobs ", response)) & isTRUE(grepl("tarrifs? | tarriffs? | tariffs? | tarifs? | trade ", response))) {
        section[i, col] = 1
        next
      } else {
        section[i, col] = 0
...
for (i in 1:nrow(section)) {
  ...
  #   if(j == 22) { 
  #     hits[[j]] <- ((section[["e_22"]][i] & section[["k_22"]][i]) == 1) || ((section[["e_22"]][i] & section[["k_23"]][i]) == 1)
  #     not_matches[[j]] <- (section[[glue("e_22")]][i] != section[[glue("k_22")]][i]) & (section[[glue("e_22")]][i] != section[[glue("k_23")]][i])
  #     next
  #   }
  #   if(j == 23) {
  #     hits[[j]] <- ((section[["e_23"]][i] & section[["k_23"]][i]) == 1) || ((section[["e_23"]][i] & section[["k_22"]][i]) == 1)
  #     not_matches[[j]] <- (section[[glue("e_23")]][i] != section[[glue("k_23")]][i]) || (section[[glue("e_23")]][i] != section[[glue("k_22")]][i])
  #     next
  #   }
  #   hits[[j]] <- (section[[glue("e_{j}")]][i] & section[[glue("k_{j}")]][i]) == 1
  #   not_matches[[j]] <- section[[glue("e_{j}")]][i] != section[[glue("k_{j}")]][i]
